    ...
    <div className="flex justify-center">
      ...
      >
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-sm text-red-400">
                {errorMessage
                  ? t('game.fallback.invalidPgn', 'PGN invalide')
                  : t('game.fallback.title', 'Partie introuvable')}
              </h2>
            </div>
            <p className="text-xs text-red-300/80">
              {errorMessage ||
                t('game.fallback.desc', 'Collez un PGN ci-dessous pour analyser une partie.')}
            </p>
          </>
        ) : (
          <>
            ...
            <p className="text-xs text-gray-400">
              {t('game.fallback.analyzeDesc', "Collez un PGN pour commencer l'analyse.")}
            </p>
          </>
        )}
        ...
        <div className="space-y-1">
          <label className="block text-xs uppercase tracking-wide text-gray-400">{t('game.fallback.pgnLabel')}</label>
          <textarea
