...
export const useChessStore = create<State & Actions>()(
  ...
  setPageSize: (n) => set({ pageSize: Math.max(1, Math.floor(n) || 15), page: 1 }),
      loadGames: async () => {
        ...
        if (!username) return
        // Abort previous in-flight fetches immediately
        try { get().abortCtl?.abort() } catch {
          // previous abort may fail silently
        }
        const newId = Date.now()
        const ctl = new AbortController()
        set({ games: [], loading: true, error: undefined, suggestions: [], page: 1, loadId: newId, abortCtl: ctl })
  ...
  const flush = () => {
          ...
          // Before updating, ensure this load is still active
          if (get().loadId !== newId) { buffer = []; return }
          const add = buffer
          buffer = []
          // update games and suggestions in one set
          set((s) => {
            ...
            // cap suggestions to avoid huge arrays
            const cap = 2000
        ...
        try {
          ...
          // Ignore abort errors (user-triggered cancellation)
          const name = (e && typeof e === 'object' && 'name' in e) ? (e as { name?: string }).name : undefined
          if (name !== 'AbortError') {
            let msg = (e && typeof e === 'object' && 'message' in e) ? (e as { message?: string }).message ?? 'Erreur inconnue' : 'Erreur inconnue'
            if (e instanceof ApiError) {
              // Store the i18n key; App.tsx will translate via t(msg)
              msg = e.code
            } else if (/Failed to fetch/i.test(msg)) {
              // Legacy normalization
              msg = 'error.offline'
            }
            const stack = (e && typeof e === 'object' && 'stack' in e) ? (e as { stack?: unknown }).stack : undefined
...
export function useFilteredGames(): Game[] {
  ...
  if (filters.modes !== 'all' && g.time_class !== filters.modes) return false
    ...
    // Hide games not involving current username (safety, should always involve)
    if (g.white.username.toLowerCase() !== username.toLowerCase() && g.black.username.toLowerCase() !== username.toLowerCase()) return false
    return true
  })
